...
class Control {
  constructor(params) {
    ...
    check.assert.string(params.lightTimeout, 'params.lightTimeout is not a string');
    check.assert.string(params.trailingTime, 'params.trailingTime is not a string');
    check.assert.string(params.minRunTime, 'params.minRunTime is not a string');
    ...
    check.assert.object(params.pins, 'params.pins is not an object');
  ...
  init() {
    const lightConfig = _.get(this.pins, ['light'], {});
    const dht22Config = _.get(this.pins, ['dht22'], {});
    const fanConfig   = _.get(this.pins, ['fan'], {});
    ...
    if(fanConfig.power && fanConfig.speed) {
      this.fanControl = new FanControl({
        ...
        default: this.fanDefault,
        // default: this.location === 'bath' ? 'off' : 'min',
      });
    }
  ...
  checkFanControl() {
    ...
    if(light.value === 'on') {
      ...
      if(moment().diff(light.since, 'millisecond') > ms(this.lightTimeout)) {
      ...
      if(moment().diff(light.since, 'millisecond') > ms(this.trailingTime)) {
    ...
    if(moment().diff(fan.since, 'millisecond') < ms(this.minRunTime)) {
      return false;
    }
    ...
    if(humidityValue) {
      const downToMinThreshold = this.humidityMinThreshold - 5;
      const downToMaxThreshold = this.humidityMaxThreshold - 10;
